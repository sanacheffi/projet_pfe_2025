...
import { convertCustomizationToOrder } from "../../redux/slices/customizationSlice";

const ConvertCustomizationForm = () => {
  const { customizationId } = useParams();                   
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const { loading, error } = useSelector((state) => state.customization);
  ...
  const handleChange = (e) => {
    ...
    // champs imbriqués dans shippingAddress
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 shadow-md rounded-md">
      ...
      <h2 className="text-3xl font-bold mb-6">
        Convertir la personnalisation en commande
      </h2>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button
          type="submit"
          className="w-full py-3 px-4 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded transition duration-200 disabled:opacity-50"
          disabled={loading}
        >
...
export default ConvertCustomizationForm;
